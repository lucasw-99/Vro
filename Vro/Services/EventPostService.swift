...
//        FollowersService.getFollowerInfo(String(eventPost.event.host.uid), userFollowersRef) { followerInfo in
//            // add this event to all followers timelines
//            var followers = followerInfo.followers
//            let currentUser = Follower(String(eventPost.event.host.uid))
//            followers.insert(1, at: 0)
//            TimelineService.addPostToTimelines(Set<Follower>(), eventPost.eventPostID, String(eventPost.event.host.uid), withUpdates: newUpdates) { finalUpdates in
