...
//
//        let newAttendee = Attendee(String(attendeeUid), eventPostId, true, false)
...
//        NotificationService.postNotification(forNotification: AttendeeNotification(eventAddress: eventPost.event.address, eventTime: eventPost.event.eventTime, attendeeUid: String(attendee.uid), seen: false, forUserUid: String(eventPost.event.host.uid), notificationId: newAttendee.identifier), withUpdates: updates) { finalUpdates in
...
//            let finalUpdates = NotificationService.removeNotification(forUser: String(eventPost.event.host.uid), notificationId: identifier, withUpdates: updateDict).mapValues { $0 as Any }
