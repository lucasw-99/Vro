...
//        let like = Like(String(uid), eventPostID)
//        let likeDict = like.dictValue
//
//        var updates = [String: Any?]()
//        updates[likesPath] = likeDict
//        NotificationService.postNotification(forNotification: LikeNotification(likedPostId: post.eventPostID, userUid: String(user.uid), seen: false, forUserUid: String(post.event.host.uid), notificationId: like.identifier), withUpdates: updates) { finalUpdates in
...
//            let finalUpdates = NotificationService.removeNotification(forUser: String(eventPost.event.host.uid), notificationId: identifier, withUpdates: updates).mapValues { $0 as Any }
