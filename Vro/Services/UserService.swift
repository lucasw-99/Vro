...
//        }
    ...
    static func createUser(_ email: String, _ username: String, _ password: String, completion: @escaping ( (_ response: DataResponse<Any>) -> () )) {
        ...
        Alamofire.request("http://178.128.183.75/users/authenticate", method: .post, parameters: parameters, encoding: JSONEncoding.default)
            .responseJSON { response in
                ...
                guard let token = data["token"] as? String,
                    let userDict = data["user"] as? [String: Any] else {
                        fatalError("Malformatted data from server!")
                }
                
                let currentUser = UserProfile(userJson: userDict, photoUrlString: Constants.newUserProfilePhotoURL)
